import { DiagnosisPESResult, Citation } from './models'

const PES_REGISTRY: Record<string, { entity_id: string; label: string; domain: string }> = {
...
const MAKANAN_KE_KODE: Record<string, string> = {
  mual: 'NI-1.2',
...
export function infer_kode_dari_keluhan(keluhan: string[], diagnosis_medis: string[]): string {
  const semua = [...keluhan.map((k) => k.toLowerCase()), ...diagnosis_medis.map((d) => d.toLowerCase())]
  for (const kata of semua) {
    for (const [key, kode] of Object.entries(MAKANAN_KE_KODE)) {
      if (kata.includes(key)) return kode
    }
  }
  return 'NI-1.2'
}

export function bangun_pernyataan(
  kode_pes: string,
  etiologi: string,
  signs: string,
): [DiagnosisPESResult, Citation[]] {
  const info = PES_REGISTRY[kode_pes] ?? PES_REGISTRY['NI-1.2']
  const pes_statement = `${kode_pes} ${info.label} related to ${etiologi} as evidenced by ${signs}`

  const result: DiagnosisPESResult = {
    problem: kode_pes,
    label: info.label,
    etiologi,
    signs,
